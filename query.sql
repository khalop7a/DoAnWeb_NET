...
go 
 ...
 -- Huy them proce xu ly dang nhap
 CREATE PROCEDURE Sp_Account_Login_CT
	@UserName varchar(20),
	@Password varchar(50)
AS
BEGIN
	declare @count int
	declare @res bit
	select @count = count(*) from ChuTro as ct where ct.ChuTro_ID = @UserName and ct.MatKhau = @Password
	if @count > 0
		set @res = 1
	else
		set @res = 0
...
CREATE PROCEDURE Sp_Account_Login_Admin
	@UserName varchar(20),
	@Password varchar(50)
AS
BEGIN
	declare @count int
	declare @res bit
	select @count = count(*) from Admin as a where a.Id = @UserName and a.MatKhau = @Password
	if @count > 0
		set @res = 1
	else
		set @res = 0
